...
void Input(int **a, const int rowCount, const int colCount, int currentRow = 0, int currentCol = 0);
void Print(int **a, const int rowCount, const int colCount, int currentRow = 0, int currentCol = 0);
int CountColumnsWithZeroRecursive(int **a, const int rowCount, const int colCount, int currentCol = 0);
bool HasZeroInColumnRecursive(int **a, const int rowCount, int col, int currentRow = 0);
int LongestSeriesInRowRecursive(int *row, const int colCount, int currentCol = 1, int currentLength = 1, int maxLength = 1);
void FindRowWithLongestSeriesRecursive(int **a, const int rowCount, const int colCount, int currentRow, int &longestRow, int &maxSeriesLength);
...
void Input(int **a, const int rowCount, const int colCount, int currentRow, int currentCol)
{
    if (currentRow == rowCount)
        return;
    ...
    if (currentCol < colCount)
    {
        ...
        cin >> a[currentRow][currentCol];
        Input(a, rowCount, colCount, currentRow, currentCol + 1);
    }
    else
    {
        cout << endl;
        Input(a, rowCount, colCount, currentRow + 1, 0);
    }
}

void Print(int **a, const int rowCount, const int colCount, int currentRow, int currentCol)
{
    if (currentRow == rowCount)
        return;
    ...
    if (currentCol < colCount)
    {
        cout << setw(4) << a[currentRow][currentCol];
        Print(a, rowCount, colCount, currentRow, currentCol + 1);
    }
    else
    {
        cout << endl;
        Print(a, rowCount, colCount, currentRow + 1, 0);
    }
}
...
// чи є нульовий елемент у стовпці
bool HasZeroInColumnRecursive(int **a, const int rowCount, int col, int currentRow)
{
    if (currentRow == rowCount)
        return false;
...
//  найдовшої серії однакових елементів у рядку
int LongestSeriesInRowRecursive(int *row, const int colCount, int currentCol, int currentLength, int maxLength)
{
    if (currentCol == colCount)
        return maxLength;
